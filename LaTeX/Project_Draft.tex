...
\subsection{Profit Model}
	The profit model for a cable company is defined by five variables: profit, $\pi$, revenue per passing, \emph{rev}, penetration rate, \emph{pen}, cost to service customer per passing, \emph{cts}, and other costs per passing \emph{oc}.
\begin{equation}
	\pi = \emph{rev} * \emph{pen} - \emph{cts}*\emph{oc}
\end{equation}
